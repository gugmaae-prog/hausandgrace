...
const clean = (value = "") => decode(value).replace(/\s*[|–-]\s*OPR(?:\.AE)?.*$/i, "").replace(/Metropolitan Premium Properties/gi, "HAUS & GRACE").replace(/OPR(?:\.AE)?/gi, "HAUS & GRACE").replace(/\s+/g, " ").trim();
...
async function fetchText(url, timeout = 45000, attempts = 2) {
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    try { const response = await fetch(url, { signal: controller.signal, headers: { "user-agent": "HAUS-GRACE-Registry/1.0" } }); if (response.ok) return await response.text(); }
