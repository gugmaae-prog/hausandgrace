...
WHERE lower(`email`) = 'mehul@hausandgrace.ae';
--> statement-breakpoint
UPDATE `hg_agent_profiles`
SET `display_name` = 'Priya Mistry',
    `title` = 'Chief Executive Officer',
    `updated_at` = CURRENT_TIMESTAMP
WHERE lower(`email`) = 'priya@hausandgrace.ae';
--> statement-breakpoint
UPDATE `hg_agent_profiles`
SET `display_name` = 'Irfan Baismail',
    `updated_at` = CURRENT_TIMESTAMP
WHERE lower(`email`) = 'irfan@hausandgrace.ae';
--> statement-breakpoint
UPDATE `hg_agent_profiles`
SET `display_name` = 'Urvashi Saraiya',
    `updated_at` = CURRENT_TIMESTAMP
WHERE lower(`email`) = 'urvashi@hausandgrace.ae';
--> statement-breakpoint
