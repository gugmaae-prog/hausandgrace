...
function wordBoundedMatch(a: string, b: string) {
  ...
  return new RegExp(`(^| )${esc(b)}( |$)`).test(a) || new RegExp(`(^| )${esc(a)}( |$)`).test(b);
}
...
export function getAreaPricePerSqft(area: string, propertyTypes: string[] = [], emirate = ""): PricePerSqft | null {
  ...
  let matches = pricePerSqftBenchmarks.filter((benchmark) =>
    benchmark.aliases.some((alias) => wordBoundedMatch(areaKey, normalize(alias))),
  );
  ...
  if (!matches.length) {
    ...
    matches = pricePerSqftBenchmarks.filter((benchmark) => benchmark.emirate === emirate && normalize(benchmark.area) === normalize(emirate));
    scope = "emirate";
  }
