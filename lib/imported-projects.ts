...
import { curatedLaunches, type LaunchUnit } from "@/data/curated-launches";
...
const trustedMediaHosts = new Set(["cdn.opr.ae", "img1.creatium.ru", "img2.creatium.ru", "img3.creatium.ru", "i.1.creatium.io", "new-projects-media.propertyfinder.com", "szr2.crimsoncapedigital.com"]);
...
function cleanBrand(value = "") { return decode(value).replace(/\s*[|–-]\s*OPR(?:\.AE)?[^|–-]*/gi, "").replace(/Metropolitan Premium Properties/gi, "HAUS & GRACE").replace(/OPR(?:\.AE)?/gi, "HAUS & GRACE").replace(/\s+/g, " ").trim(); }
...
const curatedRecords: RegistryProject[] = curatedLaunches.map((launch) => ({
  slug: launch.slug,
  name: launch.name,
...
const mergedProjects = [...curatedRecords, ...rawRegistry.projects.filter((project) => !curatedSlugs.has(project.slug))];
...
const curatedMap = new Map(curatedLaunches.map((project) => [project.slug, project]));
...
export function getRelatedProjectRecords(project: ImportedProject, limit = 3): RegistryProject[] {
  ...
  return registry.projects
    .filter((candidate) => candidate.slug !== project.slug && !candidate.archived && usableImage(candidate.image))
    .map((candidate) => {
      let score = 0;
...
export async function getImportedProject(slug: string): Promise<ImportedProject | null> {
  const sourceSlug = normalizeProjectSlug(slug); const record = getProjectRecord(sourceSlug); if (!record) return null;
  const curated = curatedMap.get(sourceSlug);
  if (curated) {
    const gallery = unique(curated.gallery.filter(usableImage));
    const hero = usableImage(curated.image) ? curated.image : gallery[0] || "";
    return {
      slug: sourceSlug,
      title: curated.name,
      ...
      hero,
      gallery: unique([hero, ...gallery].filter(Boolean)),
      interiors: curated.interiors.filter(usableImage),
      exteriors: curated.exteriors.filter(usableImage),
      floorplans: curated.floorplans.filter(usableImage),
  ...
  let html = ""; try { const response = await fetch(`${SOURCE}/projects/${sourceSlug}`, { next: { revalidate: 21600 }, headers: { "user-agent": "HAUS-GRACE-Catalogue/2.0" } }); if (response.ok) html = await readLimitedHtml(response); } catch { /* registry remains a complete fallback */ }
  ...
  const description = cleanBrand(meta(html, "description") || meta(html, "og:description") || record.description || `Discover ${title}, a considered new address in ${record.emirate}.`);
  const allImages = imagesIn(html); const sourceHero = normalizeImage(meta(html, "og:image")); const hero = (usableImage(sourceHero) ? sourceHero : "") || (usableImage(record.image) ? record.image : "") || allImages[0] || "";
  const labelled = labelledGallery(html); const floorplans = floorplanImages(html, allImages);
  const labelledMediaAvailable = labelled.interiors.length > 0 || labelled.exteriors.length > 0;
  let interiors = labelled.interiors.length ? labelled.interiors : allImages.filter((url) => /(interior|living|bedroom|kitchen|lobby|bathroom|dining|lounge|reception|amenit)/i.test(url));
  let exteriors = labelled.exteriors.length ? labelled.exteriors : allImages.filter((url) => /(exterior|facade|fa[cç]ade|elevation|tower|villa|aerial|masterplan|building|residence|waterfront|landscape)/i.test(url));
  ...
  const unclassified = labelledMediaAvailable ? [] : allImages.filter((url) => !floorplans.includes(url) && !interiors.includes(url) && !exteriors.includes(url));
  ...
  const gallery = unique([hero, ...exteriors, ...interiors]);
